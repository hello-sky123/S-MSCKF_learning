...
ImageProcessor::~ImageProcessor() { destroyAllWindows(); }

bool ImageProcessor::loadParameters() {
  // Camera calibration parameters
  // 如果参数服务器上存在指定参数，则返回其值；如果不存在，则返回提供的默认值
  nh.param<string>("cam0/distortion_model", cam0_distortion_model,
                   string("radtan"));
  nh.param<string>("cam1/distortion_model", cam1_distortion_model,
                   string("radtan"));
  ...
  nh.param<int>("grid_min_feature_num", processor_config.grid_min_feature_num,
                2);
  nh.param<int>("grid_max_feature_num", processor_config.grid_max_feature_num,
                4);
  ...
  ROS_INFO("cam0_intrinscs: %f, %f, %f, %f", cam0_intrinsics[0],
  ...
  ROS_INFO("cam1_intrinscs: %f, %f, %f, %f", cam1_intrinsics[0],
...
void ImageProcessor::stereoCallback(
  ...
  cam0_curr_img_ptr =
      cv_bridge::toCvShare(cam0_img, sensor_msgs::image_encodings::MONO8);
  cam1_curr_img_ptr =
      cv_bridge::toCvShare(cam1_img, sensor_msgs::image_encodings::MONO8);
  ...
  for (int code = 0;
       code < processor_config.grid_row * processor_config.grid_col; ++code) {
...
void ImageProcessor::predictFeatureTracking(
  ...
  // Intrinsic matrix.
  cv::Matx33f K(intrinsics[0], 0.0, intrinsics[2], 0.0, intrinsics[1],
...
void ImageProcessor::trackFeatures() {
  ...
  predictFeatureTracking(prev_cam0_points, cam0_R_p_c, cam0_intrinsics,
                         curr_cam0_points);

  calcOpticalFlowPyrLK(
      prev_cam0_pyramid_, curr_cam0_pyramid_, prev_cam0_points,
      curr_cam0_points, track_inliers, noArray(),
      Size(processor_config.patch_size, processor_config.patch_size),
  ...
  removeUnmarkedElements(prev_cam0_points, track_inliers,
                         prev_tracked_cam0_points);
  removeUnmarkedElements(prev_cam1_points, track_inliers,
                         prev_tracked_cam1_points);
  removeUnmarkedElements(curr_cam0_points, track_inliers,
  ...
  removeUnmarkedElements(prev_tracked_lifetime, match_inliers,
                         prev_matched_lifetime);
  removeUnmarkedElements(prev_tracked_cam0_points, match_inliers,
                         prev_matched_cam0_points);
  removeUnmarkedElements(prev_tracked_cam1_points, match_inliers,
                         prev_matched_cam1_points);
  removeUnmarkedElements(curr_tracked_cam0_points, match_inliers,
                         curr_matched_cam0_points);
  removeUnmarkedElements(curr_cam1_points, match_inliers,
  ...
  twoPointRansac(prev_matched_cam0_points, curr_matched_cam0_points, cam0_R_p_c,
                 cam0_intrinsics, cam0_distortion_model, cam0_distortion_coeffs,
                 processor_config.ransac_threshold, 0.99, cam0_ransac_inliers);

  vector<int> cam1_ransac_inliers(0);
  twoPointRansac(prev_matched_cam1_points, curr_matched_cam1_points, cam1_R_p_c,
                 cam1_intrinsics, cam1_distortion_model, cam1_distortion_coeffs,
                 processor_config.ransac_threshold, 0.99, cam1_ransac_inliers);
  ...
  ROS_INFO_THROTTLE(0.5,
                    "\033[0;32m candidates: %d; track: %d; match: %d; ransac: "
                    "%d/%d=%f\033[0m",
  ...
  // printf(
  //    "\033[0;32m candidates: %d; raw track: %d; stereo match: %d; ransac:
  //    %d/%d=%f\033[0m\n", before_tracking, after_tracking, after_matching,
  //    curr_feature_num, prev_feature_num,
  //    static_cast<double>(curr_feature_num)/
  //    (static_cast<double>(prev_feature_num)+1e-5));
...
void ImageProcessor::stereoMatch(const vector<cv::Point2f>& cam0_points,
  ...
  // Track features using LK optical flow method.
  calcOpticalFlowPyrLK(
      curr_cam0_pyramid_, curr_cam1_pyramid_, cam0_points, cam1_points,
  ...
  for (int i = 0; i < cam0_points_undistorted.size(); ++i) {
    if (inlier_markers[i] == 0) continue;
    cv::Vec3d pt0(cam0_points_undistorted[i].x, cam0_points_undistorted[i].y,
                  1.0);
    cv::Vec3d pt1(cam1_points_undistorted[i].x, cam1_points_undistorted[i].y,
                  1.0);
...
void ImageProcessor::addNewFeatures() {
  ...
  for (const auto& features : *curr_features_ptr) {
    for (const auto& feature : features.second) {
      ...
      int up_lim = y - 2, bottom_lim = y + 3, left_lim = x - 2,
          right_lim = x + 3;
  ...
  vector<vector<KeyPoint> > new_feature_sieve(processor_config.grid_row *
  ...
  for (auto& item : new_feature_sieve) {
    if (item.size() > processor_config.grid_max_feature_num) {
      ...
      item.erase(item.begin() + processor_config.grid_max_feature_num,
                 item.end());
    }
  ...
  for (int code = 0;
    ...
    int vacancy_num =
        processor_config.grid_min_feature_num - features_this_grid.size();
  ...
  // printf("\033[0;33m detected: %d; matched: %d; new added feature:
  // %d\033[0m\n",
...
void ImageProcessor::pruneGridFeatures() {
  for (auto& item : *curr_features_ptr) {
    ...
    grid_features.erase(
        grid_features.begin() + processor_config.grid_max_feature_num,
        grid_features.end());
  }
  return;
}
...
void ImageProcessor::undistortPoints(const vector<cv::Point2f>& pts_in,
  ...
  const cv::Matx33d K(intrinsics[0], 0.0, intrinsics[2], 0.0, intrinsics[1],
                      intrinsics[3], 0.0, 0.0, 1.0);

  const cv::Matx33d K_new(new_intrinsics[0], 0.0, new_intrinsics[2], 0.0,
...
vector<cv::Point2f> ImageProcessor::distortPoints(
    const vector<cv::Point2f>& pts_in, const cv::Vec4d& intrinsics,
...
void ImageProcessor::integrateImuData(Matx33f& cam0_R_p_c,
  ...
  while (begin_iter != imu_msg_buffer.end()) {
    if ((begin_iter->header.stamp - cam0_prev_img_ptr->header.stamp).toSec() <
        -0.01)
      ++begin_iter;
    else
      break;
  ...
  while (end_iter != imu_msg_buffer.end()) {
    if ((end_iter->header.stamp - cam0_curr_img_ptr->header.stamp).toSec() <
        0.005)
      ++end_iter;
    else
      break;
  ...
  double dtime =
      (cam0_curr_img_ptr->header.stamp - cam0_prev_img_ptr->header.stamp)
          .toSec();
...
void ImageProcessor::twoPointRansac(
  ...
  if (pts1.size() != pts2.size())
    ROS_ERROR("Sets of different size (%lu and %lu) are used...", pts1.size(),
              pts2.size());
  ...
  undistortPoints(pts1, intrinsics, distortion_model, distortion_coeffs,
                  pts1_undistorted);
  undistortPoints(pts2, intrinsics, distortion_model, distortion_coeffs,
                  pts2_undistorted);
  ...
  for (int iter_idx = 0; iter_idx < iter_num; ++iter_idx) {
    ...
    int select_idx_diff =
        random_gen.uniformInteger(1, raw_inlier_idx.size() - 1);
    int select_idx2 =
...
void ImageProcessor::drawFeaturesStereo() {
  if (debug_stereo_pub.getNumSubscribers() > 0) {
    ...
    cvtColor(cam0_curr_img_ptr->image, out_img.colRange(0, img_width),
             CV_GRAY2RGB);
    ...
    for (const auto& new_cam0_point : curr_cam0_points) {
      ...
      cv::Point2f pt1 =
          curr_cam1_points[new_cam0_point.first] + Point2f(img_width, 0.0);
