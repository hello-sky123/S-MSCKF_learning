...
// 以Eigen::Isometry3d(刚体变换)格式返回变换参数
...
cv::Mat getTransformCV(const ros::NodeHandle& nh, const std::string& field) {
  ...
  try {
    ...
    ROS_DEBUG_STREAM("cannot read transform "
                     << field << " in kalibr format, trying old one!");
    try {
...
// 读取kalibr格式的transform参数，按每行4个元素，四行的形式存储在[]中
